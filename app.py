    ...
    while True:
        ...
        if s == "complete":
            ...
            with content_ph.container():
                ...
                DOC_EXTS      = {".pdf"}
                TEXT_EXTS     = {".txt", ".eml"}
                UNSUPPORTED   = {".docx", ".doc"}

                if file_ext:
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown("""
                    <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:600;margin-bottom:12px;">
                        Source File
                    </div>
                    """, unsafe_allow_html=True)

                    # File info chip
                    ext_label = file_ext.lstrip(".").upper()
                    if file_ext in AUDIO_EXTS:
                        type_icon, type_color = "🎙", "var(--blue)"
                        type_label = "Audio Recording"
                    elif file_ext in DOC_EXTS:
                        type_icon, type_color = "📄", "var(--gold)"
                        type_label = "PDF Document"
                    elif file_ext in TEXT_EXTS:
                        type_icon, type_color = "✉", "var(--amber)"
                        type_label = "Email / Text"
                    else:
                        type_icon, type_color = "📎", "var(--text-muted)"
                        type_label = "Document"
                    ...
                    <div style="display:flex;align-items:center;gap:12px;
                      ...
                      <div style="flex:1;">
                        ...
                        <div style="font-size:11px;color:var(--text-muted);margin-top:2px;font-family:'DM Mono',monospace;">
                          {type_label} &nbsp;·&nbsp; {ext_label}
                        </div>
                      </div>
                    </div>
                    """, unsafe_allow_html=True)

                    # Fetch source bytes for preview/download
                    source_bytes = api_get_bytes(f"/download/{job_id}/source")

                    if source_bytes:
                        if file_ext in AUDIO_EXTS:
                            # Native audio player — Streamlit renders this inline
                            mime = "audio/mpeg" if file_ext == ".mp3" else (
                                   "audio/wav"  if file_ext == ".wav" else "audio/mp4")
                            st.audio(source_bytes, format=mime)
                            st.download_button(
                                f"⬇ Download {ext_label}",
                                source_bytes,
                                file_name=file_name,
                                mime=mime,
                                key="dl_source_audio",
                            ...
                            with st.expander("👁  Preview source text", expanded=False):
                                try:
                                    ...
                                    st.warning("Could not decode text content.")
                            st.download_button(
                                f"⬇ Download {ext_label}",
                                source_bytes,
                                file_name=file_name,
                        ...
                        elif file_ext in UNSUPPORTED:
                            # DOCX can't be rendered in browser — just offer download
                            st.markdown(f"""
                            <div style="background:var(--surface2);border:1px solid var(--border);
                                 border-radius:8px;padding:12px 16px;font-size:12px;color:var(--text-muted);">
                              In-browser preview is not available for Word documents.
